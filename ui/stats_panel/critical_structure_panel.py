...
class CriticalStructurePanel(BaseStatsPanel):
                ...
                try:
                    ...
                    for source in range(n_layers):
                        ...
                        for _ in range(3):  # 3 steps of diffusion
                            new_flow = {i: 0.0 for i in range(n_layers) if i in G.nodes()}
                            for i in flow:
                                if flow[i] > 0:
                    ...
                    for i in range(n_layers):
                        ...
                        flow_without_i = {}
                        for source in range(n_layers):
                            if source != i and source in G_without_i.nodes():  # Skip the removed layer as source
                                # Initialize with all flow at source
                                flow = {j: 0.0 for j in range(n_layers) if j != i and j in G_without_i.nodes()}
                                flow[source] = 1.0
                                
                                # Simulate random walk for a few steps
                                for _ in range(3):  # 3 steps of diffusion
                                    new_flow = {j: 0.0 for j in range(n_layers) if j != i and j in G_without_i.nodes()}
                                    for j in flow:
                                        if flow[j] > 0:
                                            # Distribute flow to neighbors
                        ...
                        if impact_data["before"] > 0:
                            impact = (impact_data["before"] - avg_flow_without_i) / impact_data["before"]
                        else:
                            impact = 0
    ...
    def create_criticality_visualizations(self, criticality_scores, impact_data, layers, medium_font, large_font, layer_colors):
        ...
        if not criticality_scores or impact_data["before"] is None:
            self.criticality_bar_ax.text(0.5, 0.5, "Not enough data for criticality analysis", 
                                   ha='center', va='center', fontsize=12)
            self.criticality_bar_ax.axis('off')
            
            self.impact_ax.text(0.5, 0.5, "Not enough data for impact visualization", 
                              ha='center', va='center', fontsize=12)
        ...
        if layer_names:  # Only create visualization if we have data
            ...
            self.create_impact_visualization(layer_indices, layer_names, impact_data, medium_font, large_font, colors)
        else:
            # No data to visualize
            self.criticality_bar_ax.text(0.5, 0.5, "No critical layers identified", 
                                   ha='center', va='center', fontsize=12)
            self.criticality_bar_ax.axis('off')
            
            self.impact_ax.text(0.5, 0.5, "No impact data to visualize", 
                              ha='center', va='center', fontsize=12)
            self.impact_ax.axis('off')
    ...
    def create_anomaly_visualizations(self, anomalies, anomaly_scores, layer_connections, layers, medium_font, large_font, layer_colors, threshold=1.0):
        ...
        # Check if we have valid data to visualize
        if not layers or n_layers < 2 or layer_connections.size == 0 or not anomaly_scores:
            self.anomaly_ax.text(0.5, 0.5, "Not enough data for anomaly detection", 
                               ha='center', va='center', fontsize=12)
            self.anomaly_ax.axis('off')
            
            self.network_ax.text(0.5, 0.5, "Not enough data for network visualization", 
                               ha='center', va='center', fontsize=12)
        ...
        if anomaly_scores:
            try:
                ...
                for i, j in anomalies:
                    if 0 <= i < n_layers and 0 <= j < n_layers:  # Ensure indices are valid
                ...
                if G.number_of_edges() == 0:
                    self.network_ax.text(0.5, 0.5, "No connections to visualize", 
                                       ha='center', va='center', fontsize=12)
                    self.network_ax.axis('off')
                    return
                ...
                self.anomaly_ax.text(0.5, 0.5, "Error creating anomaly visualization", 
                                   ha='center', va='center', fontsize=12)
                self.anomaly_ax.axis('off')
                
                self.network_ax.text(0.5, 0.5, "Error creating network visualization", 
                                   ha='center', va='center', fontsize=12)
                self.network_ax.axis('off')
        else:
    ...
    def create_impact_visualization(self, layer_indices, layer_names, impact_data, medium_font, large_font, colors):
        """Create visualization for impact of layer removal"""
        if not layer_indices or impact_data["before"] is None or not impact_data["after"]:
            self.impact_ax.text(0.5, 0.5, "No impact data to visualize", 
                              ha='center', va='center', fontsize=12)
            self.impact_ax.axis('off')
            return
        ...
        try:
            ...
            top_colors = colors[:layers_to_show] if colors else ['skyblue'] * layers_to_show
            ...
            self.impact_ax.text(0.5, 0.5, "Error creating impact visualization", 
                              ha='center', va='center', fontsize=12)
            self.impact_ax.axis('off')

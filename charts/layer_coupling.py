...
def create_layer_coupling_charts(coupling_heatmap_ax, coupling_bar_ax, dendrogram_ax, circular_ax, 
                               layer_connections, layers, medium_font=None, large_font=None):
    ...
    coupling_matrix = calculate_coupling_metric(layer_connections, metric="Edge Density")
    ...
    bars = coupling_bar_ax.barh(range(len(sorted_layers)), sorted_scores, color='skyblue')
    ...
    for i, bar in enumerate(bars):
        width = bar.get_width()
        coupling_bar_ax.text(width * 1.01, bar.get_y() + bar.get_height()/2, 
    ...
    nx.draw_networkx_nodes(G, pos, node_size=1000, 
                          node_color='lightblue', alpha=0.6, ax=circular_ax)
    ...
    if edges:
        ...
        nx.draw_networkx_edges(G, pos, width=edge_widths, 
                             ...
                             edge_cmap=plt.cm.viridis, ax=circular_ax)

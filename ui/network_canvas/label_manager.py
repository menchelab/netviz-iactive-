...
class LabelManager:
    ...
    def _update_labels_and_bars(self, node_mask, interlayer_edge_counts, show_labels, show_stats_bars):
        ...
        if (self.canvas.data_manager.node_ids is None or self.canvas.data_manager.visible_layers is None or 
            self.canvas.data_manager.layer_names is None or not self.canvas.data_manager.visible_layers):
        ...
        top_layer_idx = min(self.canvas.data_manager.visible_layers) if self.canvas.data_manager.visible_layers else -1
        ...
        for base_node, indices in base_node_to_indices.items():
            ...
            for idx in indices:
                ...
                if node_layer_idx in self.canvas.data_manager.visible_layers and node_layer_idx < top_node_layer:
                    top_node_idx = idx
            ...
            active_node_count = self.canvas.data_manager.count_active_nodes_for_base(base_node, idx)
            ...
            if show_labels:
                ...
                if self.canvas.data_manager.layer_colors_rgba and layer_name in self.canvas.data_manager.layer_colors_rgba:
        ...
        if show_stats_bars and edge_count_line_points:
            ...
            self.canvas.edge_count_bars.order = 0.9  # Draw behind labels but above node count bars
        else:
            self.canvas.edge_count_bars.visible = False
        
        logger.info(f"Label visibility set to {self.canvas.node_labels.visible}, showing {len(label_positions)} labels")
        logger.info(f"Bar visibility set to {self.canvas.node_count_bars.visible} and {self.canvas.edge_count_bars.visible}") 
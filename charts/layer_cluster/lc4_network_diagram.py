...
def create_layer_cluster_network_diagram(
    ...
    logger.info(
        f"Creating layer-cluster network diagram with layout={layout_algorithm}"
    )
    ...
    ax.clear()
    ax.set_title(
        f"Layer-Cluster Network ({layout_algorithm.title()} Layout)",
    ...
    for layer in unique_layers:
        layer_count = sum(
            cluster_layer_counts[cluster][layers.index(layer)]
            for cluster in unique_clusters
    ...
    # Draw layer labels with semi-transparent background
    for node, label in layer_labels.items():
        x, y = pos[node]
        ax.text(
            x,
            y,
    ...
    # Draw cluster labels with semi-transparent background
    ...
    # Add the top edges table overlay
    _create_top_edges_table(ax, G, small_fontsize)
    ...
    logger.info(
        f"Successfully created layer-cluster network with {len(unique_layers)} layers and {len(unique_clusters)} clusters"
    )
...
def _create_bipartite_layout(G, layer_nodes, cluster_nodes, aspect_ratio=1.0):
    ...
    for node in G.nodes():
        node_weights[node] = sum(
            G.edges[node, neighbor]["weight"] for neighbor in G.neighbors(node)
        )

    # Sort nodes by weight (descending)
    sorted_layer_nodes = sorted(
        layer_nodes, key=lambda n: node_weights.get(n, 0), reverse=True
    )
    sorted_cluster_nodes = sorted(
        cluster_nodes, key=lambda n: node_weights.get(n, 0), reverse=True
    )
    ...
    for node in sorted_layer_nodes:
        ...
        height = (
            0.8 * (weight / total_layer_weight)
            if total_layer_weight > 0
            else 0.8 / len(layer_nodes)
        )
    ...
    for node in sorted_cluster_nodes:
        weight = node_weights.get(node, 0)
        height = (
            0.8 * (weight / total_cluster_weight)
            if total_cluster_weight > 0
            else 0.8 / len(cluster_nodes)
        )
...
def _adjust_spring_layout(pos, layer_nodes, cluster_nodes, aspect_ratio=1.0):
    ...
    for node in pos:
        pos[node][1] = (
            0.1 + 0.8 * (pos[node][1] - y_min) / y_range if y_range > 0 else 0.5
        )
...
def _create_top_edges_table(ax, G, small_fontsize):
    """
    Create an overlay table in the top left showing the top 10 edges by weight.
    ...
    # Take top 10 edges
    top_edges = edges_with_weights[:10]

    # Create table data
    table_data = []
    for u, v, weight in top_edges:
        # Clean up node names for display
        source = u.replace("L_", "").replace("C_", "Cluster ")
        target = v.replace("L_", "").replace("C_", "Cluster ")
        table_data.append([source, target, f"{weight}"])

    # Create the table
    table_width = 0.3
    table_height = 0.3
    table_x = 0.01  # Position in the top left
    table_y = 0.99 - table_height  # Position from the top

    # Create the table with column headers
    table = ax.table(
        cellText=table_data,
        colLabels=["From", "To", "Strength"],
        loc="upper left",
    ...
    table.set_fontsize(small_fontsize - 1)  # Slightly smaller than small_fontsize

...
def create_cluster_sankey_chart(
    ax,
    visible_links,
    node_ids,
    node_clusters,
    ...
    if visible_layer_indices is not None and len(visible_layer_indices) > 0:
        ...
        logger.info(
            f"Filtering Sankey diagram to show only {len(visible_layers)} visible layers"
        )
    ...
    try:
        ...
        src_current_positions = {
            layer: pos for layer, pos in src_y_start_positions.items()
        }
        tgt_current_positions = {
            layer: pos for layer, pos in tgt_y_start_positions.items()
        }
            ...
            for tgt_layer, clusters in targets.items():
                ...
                for cluster, count in sorted(clusters.items(), key=lambda x: x[0]):
                    ...
                    if flow_height > 0.1:
                        ...
                        label_x = 5
                        label_y = (
                            src_y + flow_height / 2 + tgt_y + flow_height / 2
                        ) / 2
                        ax.text(
                            label_x,
                            label_y,
        ...
        ax.legend(
            ...
            ncol=min(
                5, len(unique_clusters)
            ),  # Adjust number of columns based on cluster count
            frameon=False,
        ...
        logger.info(
            f"Created custom Sankey diagram with {len(connection_matrix)} source layers and {len(unique_clusters)} clusters"
        )

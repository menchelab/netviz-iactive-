...
def create_layer_cluster_sankey(
    ...
    logger.info(
        f"Creating layer-cluster Sankey diagram with nodes_per_layer={nodes_per_layer}"
    )
    logger.info(
    ...
    logger.info(
        f"Found {len(unique_layers)} unique layers and {len(unique_clusters)} unique clusters"
    )
    ...
    try:
        ...
        sorted_layers = sorted(
            layer_totals.keys(), key=lambda x: layer_totals[x], reverse=True
        )
        sorted_clusters = sorted(
            cluster_totals.keys(), key=lambda x: cluster_totals[x], reverse=True
        )
        ...
        logger.info(
            f"Filtered to {len(filtered_flows)} flows between top layers and clusters"
        )
        ...
        logger.info(
            f"Successfully created Sankey diagram with {len(sorted_layers)} layers and {len(sorted_clusters)} clusters"
        )

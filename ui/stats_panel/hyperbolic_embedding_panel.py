...
class HyperbolicEmbeddingPanel(BaseStatsPanel):
    ...
    def setup_ui(self):
        ...
        self.embedding_method_dropdown.currentTextChanged.connect(self.on_method_changed)
    ...
    def visualize_hyperbolic_embedding(self, embedding, hierarchy_data, layers, medium_font, large_font, 
        ...
        circle = plt.Circle((0, 0), 1, fill=False, edgecolor='black', linestyle='--', alpha=0.5)
        ...
        if coloring == "Layer Colors":
            ...
            for i in range(len(layers)):
                for j in range(i+1, len(layers)):
                    ...
                    d = np.acosh(1 + 2 * ((x1-x2)**2 + (y1-y2)**2) / ((1-x1**2-y1**2) * (1-x2**2-y2**2)))
            ...
            try:
                ...
                node_colors = [plt.cm.plasma(centrality.get(i, 0) / max_centrality) for i in range(len(layers))]
            except:
                # Fallback
            ...
            if communities:
                ...
                color_map = {comm: plt.cm.tab10(i % 10) for i, comm in enumerate(unique_communities)}
                node_colors = [color_map.get(communities.get(i, 0), 'skyblue') for i in range(len(layers))]
            else:
                node_colors = ['skyblue' for _ in range(len(layers))]
        ...
        # Draw nodes
        self.ax.scatter(x_coords, y_coords, s=node_sizes, c=node_colors, alpha=0.8, edgecolors='black', zorder=10)
        ...
        for i in range(len(layers)):
            for j in range(i+1, len(layers)):
                ...
                if not np.isnan(d) and d < 2:  # Only draw edges for nearby nodes
                    ...
                    self.ax.plot([x1, x2], [y1, y2], 'gray', alpha=0.3, linewidth=1, zorder=1)
        
        # Draw node labels
        for i in range(len(layers)):
            x, y = transformed_embedding[i]
            self.ax.text(x, y, layers[i], fontsize=8, ha='center', va='center', 
        ...
        self.ax.text(0.5, -0.05, legend_text, transform=self.ax.transAxes, 
                   ha='center', va='center', fontsize=8, 
                   bbox=dict(facecolor='white', alpha=0.7))
        
        # Add navigation instructions
        nav_text = (f"Zoom: {self._zoom_level:.1f}x | Rotation: {self._rotation_angle:.0f}°\n"
                   f"Use controls above to navigate")
        self.ax.text(0.5, 1.05, nav_text, transform=self.ax.transAxes, 
                   ha='center', va='center', fontsize=8, 
                   bbox=dict(facecolor='white', alpha=0.7))

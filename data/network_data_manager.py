...
class NetworkDataManager:
    ...
    def load_data(self, node_positions, link_pairs, link_colors, node_ids, layers, 
                 ...
                 unique_origins=None, layer_colors=None):
    ...
    def get_networkx_graph(self, visible_only=True):
        ...
        if self.networkx_graph is not None and visible_only == self._last_nx_visible_only:
        ...
        for i, node_id in enumerate(self.node_ids):
            if not visible_only or self.current_node_mask[i]:
                layer_idx = i // self.nodes_per_layer
                G.add_node(i, id=node_id, layer=layer_idx, 
    ...
    def count_active_nodes_for_base(self, base_node, node_idx):
        ...
        for layer_idx in self.visible_layers:
            node_idx_in_layer = layer_idx * self.nodes_per_layer + (node_idx % self.nodes_per_layer)
            if (node_idx_in_layer < len(self.node_positions) and 
                self.current_node_mask[node_idx_in_layer] and 

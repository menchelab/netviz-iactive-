...
def create_cluster_heatmap(ax, visible_links, node_ids, node_clusters, nodes_per_layer, 
    ...
    try:
        ...
        cbar.ax.set_ylabel('Number of connections', rotation=-90, va='bottom', **small_font)
        ...
        for i in range(n_layers):
            for j in range(n_layers):
                if connection_matrix[i, j] > 0:
                    ...
                    sizes = [cluster_counts[cluster] for cluster in unique_clusters if cluster in cluster_counts]
                    colors = [cluster_colors[cluster] for cluster in unique_clusters if cluster in cluster_counts]
                    ...
                    ax.pie(sizes, colors=colors, radius=size, 
        ...
        logger.info(f"Created cluster heatmap with {n_layers} layers and {len(unique_clusters)} clusters")
        
    except Exception as e:
        logger.error(f"Error creating cluster heatmap: {str(e)}")
        ax.clear()
        ax.text(0.5, 0.5, f'Error creating cluster heatmap: {str(e)}',
                horizontalalignment='center', verticalalignment='center')
        ax.axis('off') 
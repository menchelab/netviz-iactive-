...
def create_layer_influence_chart(bar_ax, network_ax, layer_connections, layers, medium_font, large_font, 
                                visible_layers=None, layer_colors=None, metric="PageRank"):
    """
    ...
    if len(filtered_layers) > 0 and np.sum(filtered_connections) > 0:
        ...
        for i, bar in enumerate(bars):
            ...
            bar_ax.text(label_x_pos, bar.get_y() + bar.get_height()/2, f'{width:.3f}',
        ...
        legend_text = (f"Node size represents {metric_name}\n"

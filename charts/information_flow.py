...
def create_information_flow_chart(flow_ax, network_ax, layer_connections, layers, medium_font, large_font, 
                                visible_layers=None, layer_colors=None, metric="Betweenness Centrality"):
    """
    ...
    if len(filtered_layers) > 0 and np.sum(filtered_connections) > 0:
        ...
        try:
            if metric == "Betweenness Centrality":
                ...
                for i in range(len(filtered_layers)):
                    for j in range(len(filtered_layers)):
                        if i != j and filtered_connections[i, j] > 0:
                            ...
                            flow_matrix[i, j] = filtered_connections[i, j] * (centrality[i] + centrality[j]) / 2
                ...
                try:
                    centrality = nx.current_flow_betweenness_centrality(G, weight='weight', normalized=True)
                except:
                    # Fallback to regular betweenness if current_flow_betweenness is not available
                    print("Warning: Flow betweenness calculation failed, falling back to regular betweenness")
                    centrality = nx.betweenness_centrality(G, weight='weight', normalized=True)
            ...
            flow_matrix = np.zeros_like(filtered_connections, dtype=float)
            centrality = {i: 1.0/len(filtered_layers) for i in range(len(filtered_layers))}
        ...
        node_sizes = total_flow * 500 / max(total_flow) if max(total_flow) > 0 else np.ones(len(filtered_layers)) * 300
        ...
        # Draw the nodes
        nx.draw_networkx_nodes(D, pos, node_size=node_sizes, node_color=node_colors, 
                              alpha=0.8, ax=network_ax)
        
        # Draw the edges with width and color based on flow strength
        edges = D.edges(data=True)
        if edges:
            edge_widths = [d['weight'] * 5 / max(flow_matrix.max(), 0.001) for _, _, d in edges]
            
            # Use a colormap for edge colors based on flow strength
            edge_colors = [plt.cm.plasma(d['weight'] / max(flow_matrix.max(), 0.001)) for _, _, d in edges]
            
            # Draw edges with arrows to show direction
            nx.draw_networkx_edges(D, pos, width=edge_widths, edge_color=edge_colors, 
                                  alpha=0.7, ax=network_ax, arrowstyle='->', arrowsize=10)
        
        # Draw labels
        nx.draw_networkx_labels(D, pos, labels={i: filtered_layers[i] for i in range(len(filtered_layers))}, 
        ...
        legend_text = ("Node size represents total information flow\n"
                      "Edge width and color represent flow strength\n"
                      "Arrows indicate flow direction")
        network_ax.text(0.5, -0.1, legend_text, transform=network_ax.transAxes, 
        ...
        try:
            if metric == "Betweenness Centrality":
                ...
                efficiency = flow_matrix.sum() / (len(filtered_layers) * (len(filtered_layers) - 1)) if len(filtered_layers) > 1 else 0
                ...
                if centrality:
                    ...
                    metrics_text = (f"Information Flow Efficiency: {efficiency:.3f}\n"
                ...
                avg_flow = flow_matrix.sum() / (len(filtered_layers) * (len(filtered_layers) - 1)) if len(filtered_layers) > 1 else 0
                
                # Identify key flow layers
                if centrality:
                    sorted_centrality = sorted(centrality.items(), key=lambda x: x[1], reverse=True)
                    top_layers = [filtered_layers[idx] for idx, _ in sorted_centrality[:min(3, len(sorted_centrality))]]
                    
                    metrics_text = (f"Average Flow: {avg_flow:.3f}\n"
                ...
                if centrality:
                    ...
                    metrics_text = f"Top Information Central Layers: {', '.join(top_layers)}"
                else:
                    metrics_text = "Insufficient data for metrics"
        except Exception as e:
        ...
        flow_ax.text(0.5, -0.15, metrics_text, transform=flow_ax.transAxes, 
                   ha='center', va='center', fontsize=8, bbox=dict(facecolor='white', alpha=0.7))

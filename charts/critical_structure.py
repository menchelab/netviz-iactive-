...
def identify_critical_layers(layer_connections, layers, metric="Connectivity Impact"):
    ...
    if metric == "Connectivity Impact":
        ...
        for i in range(n_layers):
            ...
            impact = (
                (total_connections - remaining_connections) / total_connections
        ...
        for i in range(n_layers):
            ...
            if H.number_of_nodes() > 1:
                ...
                impact = (
                    sum(
                        abs(baseline_centrality.get(n, 0) - new_centrality.get(n, 0))
                        for n in H.nodes()
                    )
                    / H.number_of_nodes()
                )
...
def create_critical_structure_charts(
    criticality_bar_ax,
    impact_ax,
    anomaly_ax,
    network_ax,
    ...
    criticality_scores, impact_data = identify_critical_layers(
        layer_connections, layers, metric="Connectivity Impact"
    )
    ...
    impact_ax.bar(
        x - width / 2, before_values, width, label="Before Removal", color="skyblue"
    )
    impact_ax.bar(
        x + width / 2, after_values, width, label="After Removal", color="salmon"
    )
    ...
    impact_ax.set_xticklabels(
        [layers[i] for i in sorted_indices[: len(x)]],
        rotation=45,
        ha="right",
        **medium_font,
    )
    ...
    anomalies, anomaly_scores = detect_anomalies(
        layer_connections, layers, threshold=1.0
    )
    ...
    nx.draw_networkx_nodes(
        G, pos, node_color=node_colors, node_size=500, alpha=0.7, ax=network_ax
    )
    ...
    nx.draw_networkx_edges(
        G, pos, edge_color=edge_colors, width=edge_widths, ax=network_ax
    )

    # Add labels
    nx.draw_networkx_labels(
        G, pos, {i: layers[i] for i in range(len(layers))}, font_size=8, ax=network_ax
    )

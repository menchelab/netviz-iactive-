...
def create_cluster_cooccurrence_network(
    ...
    if visible_layer_indices is not None:
        ...
        logger.info(f"Filtering co-occurrence network to show only {len(visible_layers)} visible layers")
    else:
    ...
    logger.info(f"Found {len(all_clusters)} unique clusters across {len(clusters_by_layer)} layers")
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Spring layout failed: {str(e)}, using circular layout instead")
        ...
        if edge_weights:
            ...
            logger.info(f"Edge weights range from {min(edge_weights)} to {max(edge_weights)}")
        else:
            normalized_weights = []
        ...
        if cluster_colors:
            ...
            node_colors = [cluster_colors.get(cluster, (0.5, 0.5, 0.5, 1.0)) for cluster in G.nodes()]
        ...
        edge_labels = {(u, v): f"{d['weight']}" for u, v, d in G.edges(data=True) if d['weight'] > 1}
        ...
        ax.set_title(f"Cluster Co-occurrence Network\n({len(all_clusters)} clusters, {len(visible_layers)} layers)", 
                    fontsize=medium_fontsize)
        
        logger.info(f"Successfully created cluster co-occurrence network with {len(all_clusters)} clusters")

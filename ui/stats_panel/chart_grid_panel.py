# run a chart for all datasets in a grid layout
# hacked together ... not good practice
...
# Note: Layout calculations are centralized in utils/calc_layout.py
# The layout positions are computed when loading data through network_builder.py,
# which uses calc_layout.py's get_layout_position function with configurable algorithms.
# This ensures consistent layout calculations across the application.
...
class ChartGridPanel(BaseStatsPanel):
    ...
    def setup_ui(self):
        ...
        self.scroll_area.setMinimumHeight(800)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self._current_chart_module = importlib.import_module(
                        f".{chart_name}", package="charts"
                    )
                    print(
                        f"Successfully imported module with relative import: .{chart_name}"
                    )
                    ...
                    try:
                        ...
                        spec = importlib.util.spec_from_file_location(
                            chart_name, module_path
                        )
                        if spec is None:
                            raise ImportError(
                                f"Could not create spec for {module_path}"
                            )

                        self._current_chart_module = importlib.util.module_from_spec(
                            spec
                        )
            ...
            for name, obj in inspect.getmembers(self._current_chart_module):
                ...
                if (
                    ...
                    and (
                        ...
                        or name.endswith("_charts")
                        or name.endswith("_diagram")
                        or name.endswith("_heatmap")
                        or name.endswith("_graph")
                        or name.endswith("_network")
                    )
                ):
                    ...
                    if algorithm_param:
                        ...
                        if options:
                            ...
                            if algorithm_param.default != inspect.Parameter.empty:
                                default_idx = (
                                    options.index(algorithm_param.default)
                                    if algorithm_param.default in options
                                    else 0
                                )
    ...
    def generate_charts(self):
        ...
        data_dir = (
            data_manager._data_dir if hasattr(data_manager, "_data_dir") else None
        )
        if not data_dir:
        ...
        selected_algorithm = (
            ...
            if self.algorithm_dropdown.isVisible()
            else None
        )
        ...
        for i, dataset in enumerate(datasets):
            ...
            # XXX super custom for datasets from Iker
            try:
                ...
                if len(dataset_data) >= 11:  # Full data with colors and origins
                    ...
                    raise ValueError(
                        f"Dataset has unexpected format: {len(dataset_data)} items"
                    )

                print(
                    f"Successfully unpacked data for {dataset}: {len(layers)} layers, {len(node_positions)} nodes"
                )
                ...
                for start_idx, end_idx in link_pairs:
                    # Get base node names
                    start_node = (
                        node_ids[start_idx].split("_")[0]
                        if start_idx < len(node_ids)
                        else f"unknown_{start_idx}"
                    )
                    end_node = (
                        node_ids[end_idx].split("_")[0]
                        if end_idx < len(node_ids)
                        else f"unknown_{end_idx}"
                    )
                    ...
                    if start_layer == end_layer:  # Intralayer connection
                        intralayer_connections[start_node] = (
                            intralayer_connections.get(start_node, 0) + 1
                        )
                        intralayer_connections[end_node] = (
                            intralayer_connections.get(end_node, 0) + 1
                        )
                    else:  # Interlayer connection
                        interlayer_connections[start_node] = (
                            interlayer_connections.get(start_node, 0) + 1
                        )
                        interlayer_connections[end_node] = (
                            interlayer_connections.get(end_node, 0) + 1
                        )
                ...
                ax.text(
                    0.5,
                    0.5,
                    f"Error processing data for {dataset}: {str(e)}",
                    ha="center",
                    va="center",
                )
                ax.axis("off")
            ...
            if self._current_chart_function.__name__ == "create_layer_coupling_charts":
                ...
                impact_ax = fig.add_subplot(gs[0, 1])  # Top right
                ...
                try:
                    ...
                    self._current_chart_function(
                        ...
                        impact_ax=impact_ax,
                        anomaly_ax=anomaly_ax,
                        network_ax=network_ax,
                    ...
                    ax.text(
                        0.5,
                        0.5,
                        f"Error generating critical structure chart for {dataset}: {str(e)}",
                        ha="center",
                        va="center",
                        wrap=True,
                    )
                ...
                try:
                    ...
                    if (
                        "layer_connections" in sig.parameters
                        and param_mapping["layer_connections"] is None
                    ):
                        print(f"Calculating layer connections for {dataset}")
                        layer_connections = np.zeros(
                            (len(layers), len(layers)), dtype=int
                        )
                        for start_idx, end_idx in link_pairs:
                            start_layer = (
                                start_idx // nodes_per_layer if nodes_per_layer else 0
                            )
                            end_layer = (
                                end_idx // nodes_per_layer if nodes_per_layer else 0
                            )
                            layer_connections[start_layer, end_layer] += 1
                            if start_layer != end_layer:
                                layer_connections[end_layer, start_layer] += 1
                    ...
                    print(
                        f"Calling {self._current_chart_function.__name__} for {dataset} with params: {list(params.keys())}"
                    )

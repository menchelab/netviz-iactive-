...
class LayerCouplingPanel(BaseStatsPanel):
        ...
        try:
            ...
            try:
                ...
                if coupling_matrix.shape[0] != len(filtered_layers):
                    print(f"Warning: Coupling matrix dimensions ({coupling_matrix.shape}) don't match filtered layers ({len(filtered_layers)})")
            ...
            try:
                ...
                self.coupling_heatmap_ax.text(0.5, 0.5, "Error creating coupling heatmap", 
                                           ha='center', va='center', fontsize=12)
                self.coupling_heatmap_ax.axis('off')
                
                self.coupling_bar_ax.text(0.5, 0.5, "Error creating coupling bar chart", 
                                        ha='center', va='center', fontsize=12)
            ...
            try:
                ...
                self.dendrogram_ax.text(0.5, 0.5, "Error creating dendrogram", 
                                      ha='center', va='center', fontsize=12)
                self.dendrogram_ax.axis('off')
                
                self.circular_ax.text(0.5, 0.5, "Error creating circular layout", 
                                    ha='center', va='center', fontsize=12)
        ...
        try:
            if metric == "Edge Density":
                ...
                try:
                    ...
                    print(f"Warning: PageRank calculation failed, using edge weights: {e}")
                    for i in range(n_layers):
                        for j in range(n_layers):
                            if i == j:
    ...
    def create_coupling_visualizations(self, coupling_matrix, layers, medium_font, large_font, layer_colors):
        ...
        if coupling_matrix is None or len(layers) == 0:
            self.coupling_heatmap_ax.text(0.5, 0.5, "No coupling data to visualize", 
                                        ha='center', va='center', fontsize=12)
            self.coupling_heatmap_ax.axis('off')
            
            self.coupling_bar_ax.text(0.5, 0.5, "No coupling data to visualize", 
                                    ha='center', va='center', fontsize=12)
            self.coupling_bar_ax.axis('off')
            return
        ...
        try:
            ...
            try:
                ...
                if sorted_layers:  # Only create bar chart if we have layers to show
                    ...
                    # No layers to show
                    self.coupling_bar_ax.text(0.5, 0.5, "No layers to display", 
                                            ha='center', va='center', fontsize=12)
                    self.coupling_bar_ax.axis('off')
            except Exception as e:
    ...
    def create_hierarchical_visualizations(self, coupling_matrix, layers, medium_font, large_font, layer_colors):
        """Create visualizations for hierarchical layer organization"""
        try:
            # Check if we have enough layers for hierarchical clustering
            if len(layers) < 2:
                self.dendrogram_ax.text(0.5, 0.5, "Need at least 2 layers for hierarchical clustering", 
                                      ha='center', va='center', fontsize=12)
                self.dendrogram_ax.axis('off')
                
                self.circular_ax.text(0.5, 0.5, "Need at least 2 layers for circular layout", 
                                    ha='center', va='center', fontsize=12)
                self.circular_ax.axis('off')
                return
            ...
            if max_coupling > 0:
                for i in range(len(layers)):
                    for j in range(len(layers)):
                        if i != j:
                            distance_matrix[i, j] = 1 - (coupling_matrix[i, j] / max_coupling)
            ...
            try:
                ...
                if "Dimensions of Z and labels must be consistent" in str(ve):
                    ...
                    print(f"Dimension mismatch: Z shape implies {linkage_matrix.shape[0] + 1} labels, but {len(layers)} provided")
                    ...
                    self.dendrogram_ax.set_title(f'Layer Hierarchy ({method.capitalize()} Linkage) - No Labels', **large_font)
                    ...
                    self.circular_ax.set_title('Simple Circular Layout (Fallback)', **large_font)
                    self.circular_ax.axis('off')
                else:
                    raise

...
def create_cluster_chord_diagram(
    ax,
    visible_links,
    node_ids,
    node_clusters,
    ...
    if visible_layer_indices is not None and len(visible_layer_indices) > 0:
        ...
        logger.info(
            f"Filtering chord diagram to show only {len(visible_layers)} visible layers"
        )
    ...
    try:
        ...
        for i, layer in enumerate(unique_layers):
            ...
            ax.text(
                ...
                else angle * 180 / math.pi + 90,
                **small_font,
            )

        # Helper function to create a curved arc between points
        def curved_arc(
            start_angle, end_angle, inner_radius, outer_radius, cluster_color
        ):
        ...
        for src_layer, targets in connection_matrix.items():
            ...
            for tgt_layer, clusters in targets.items():
                ...
                base_width = (
                    0.1 * (total_connections / max_connections)
                ...
                for cluster, count in sorted(clusters.items(), key=lambda x: x[0]):
                    ...
                    src_x, src_y = (
                        inner_radius * math.cos(src_mid),
                        inner_radius * math.sin(src_mid),
                    )
                    tgt_x, tgt_y = (
                        inner_radius * math.cos(tgt_mid),
                        inner_radius * math.sin(tgt_mid),
                    )
                    ...
                    # Create the path
                    verts = [(src_x, src_y), (control_x, control_y), (tgt_x, tgt_y)]
                    codes = [Path.MOVETO, Path.CURVE3, Path.CURVE3]
                    path = Path(verts, codes)

                    # Add the path with the cluster color
                    width = max(
                        1, 5 * (count / max_connections)
                    )  # Scale width by connection count
                    ax.add_patch(
                        mpatches.PathPatch(
                            path,
                            facecolor="none",
        ...
        logger.info(
            f"Created chord diagram with {len(connection_matrix)} layers and {len(unique_clusters)} clusters"
        )

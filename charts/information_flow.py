...
def create_information_flow_chart(flow_ax, network_ax, layer_connections, layers, medium_font, large_font, 
    ...
    if len(filtered_layers) > 0 and np.sum(filtered_connections) > 0:
        ...
        # Add colorbar
        cbar = plt.colorbar(im, ax=flow_ax, fraction=0.046, pad=0.04)
        ...
        if metric == "Betweenness Centrality":
            ...
            efficiency = flow_matrix.sum() / (len(filtered_layers) * (len(filtered_layers) - 1))
            ...
            avg_flow = flow_matrix.sum() / (len(filtered_layers) * (len(filtered_layers) - 1))

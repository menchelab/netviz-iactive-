...
def get_available_datasets(data_dir):
    ...
    if os.path.exists(
        os.path.join(data_dir, "hetionet-v1.0-nodes.tsv")
    ) and os.path.exists(os.path.join(data_dir, "hetionet-v1.0-edges.sif")):
...
def load_dataset(
    ...
    if dataset_name == "Hetionet":
        ...
        node_metadata_path = os.path.join(
            data_dir, f"{dataset_name}_Multiplex_Metadata.tsv"
        )
